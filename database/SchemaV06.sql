-- ============================================================================
-- SCHEMA V06 – PURCHASE REQUEST, PURCHASE ORDER, STOCK MOVEMENT
-- ============================================================================
-- Built on top of V05 concepts with the following changes applied:
--
--   1. system_enum table (NEW)
--        – Single lookup table for all fixed-option fields across the system
--        – Groups: PR_STATUS, PO_STATUS, MOVEMENT_TYPE
...
--
--   2. quantity table (DROPPED)
--        – Was a detour that added a join without adding value
--        – quantity_id column removed from inventory
--        – quantity column added directly onto inventory instead
--        – balance_qty = minimum stock threshold (reorder point / stockout trigger)
--        – quantity    = actual live stock count (updated by backend on every movement)
--
--   3. stock_movement table (NEW)
--        – Full ledger of every stock change for every inventory item
--        – Source of truth for stock history and predictive analytics
--        – Links to purchase_order_item via poi_id when type = 'po_receipt'
--        – quantity_before / quantity_after enable history reconstruction
--          without running sums
--        – IMPORTANT: Created AFTER purchase_order_item (STEP 13) because
--          it holds a FK reference to purchase_order_item.poi_id
--
--   4. purchase_request_item (carried from V05 refinements)
--        – No item_no; ordering by pri_id ASC
--        – pri_quantity (renamed from quantity)
--        – item_description and uom are snapshot copies from inventory
--
--   5. purchase_order_item (carried from V05 refinements)
--        – No item_no; ordering by poi_id ASC
--        – poi_quantity (renamed from quantity)
--        – received_quantity for actual goods arrived
--        – item_description and uom are snapshot copies from inventory
--        – unit_price, discount, line_total for pricing
--
-- ============================================================================
...
-- Frontend calls GET /api/enums?group=<enum_group> to populate dropdowns.
-- Adding a new option = one INSERT row, no schema migration needed.
-- The CHECK constraints on referencing columns remain as a DB-level safety net.
--
...
CREATE TABLE system_enum (
    ...
    -- Groups related options together, e.g. 'PR_STATUS', 'PO_STATUS'
    enum_group VARCHAR(50)  NOT NULL,

    -- The stored value written into the referencing column, e.g. 'draft'
    enum_code  VARCHAR(50)  NOT NULL,

    -- Human-readable label shown in UI dropdowns, e.g. 'Draft'
    enum_label VARCHAR(100) NOT NULL,

    -- Controls display order in dropdowns (lower number = appears first)
    sort_order INTEGER      NOT NULL DEFAULT 0,

    -- Soft-disable an option without deleting it.
    -- Inactive options are hidden from dropdowns but preserved in existing records.
...
INSERT INTO system_enum (enum_group, enum_code, enum_label, sort_order) VALUES
    ...
    ('PR_STATUS', 'received', 'Received', 3),  -- supplier acknowledged the request
    ('PR_STATUS', 'closed',   'Closed',   4);  -- converted to a PO, or cancelled

-- ── PO_STATUS ──────────────────────────────────────────────────────
INSERT INTO system_enum (enum_group, enum_code, enum_label, sort_order) VALUES
    ('PO_STATUS', 'draft',     'Draft',     1),  -- being prepared, not yet sent
    ('PO_STATUS', 'sent',      'Sent',      2),  -- dispatched to supplier
    ('PO_STATUS', 'confirmed', 'Confirmed', 3),  -- supplier confirmed fulfilment
    ('PO_STATUS', 'received',  'Received',  4),  -- goods physically arrived → stock update triggered
    ('PO_STATUS', 'closed',    'Closed',    5);  -- fully completed or cancelled

-- ── MOVEMENT_TYPE ──────────────────────────────────────────────────
INSERT INTO system_enum (enum_group, enum_code, enum_label, sort_order) VALUES
    ('MOVEMENT_TYPE', 'opening',    'Opening Balance', 1),  -- initial stock when item first added
    ('MOVEMENT_TYPE', 'po_receipt', 'PO Receipt',      2),  -- stock in from a Purchase Order
    ('MOVEMENT_TYPE', 'adjustment', 'Adjustment',      3),  -- manual correction (damage, count fix)
    ('MOVEMENT_TYPE', 'sale',       'Sale',            4),  -- stock out for a sale (future use)
...
-- ==============================================
-- The old quantity table is removed. Stock data now lives directly here:
--
...
--                 This is the core input for the predictive analytics feature.
--
CREATE TABLE inventory (
    ...
    -- Actual live stock count (cache updated on every stock_movement insert)
    quantity          DECIMAL      NOT NULL DEFAULT 0,

    -- Minimum stock threshold — reorder point / stockout trigger
...
-- ==============================================
-- STEP 11: CREATE PURCHASE REQUEST TABLES
-- ==============================================
...
-- (Step number corrected — stock_movement moved to STEP 13 after purchase_order_item)
-- ==============================================
...
CREATE TABLE purchase_request (
    ...
    -- Reference number manually typed by user (e.g. supplier's ref, internal ref)
    reference_no VARCHAR(100),

    -- Payment / delivery terms, e.g. "30 days", "Net 45 days"
    terms        VARCHAR(100),

    -- Which supplier this PR is addressed to
...
-- item_description and uom are snapshot copies frozen at line-creation time —
-- they do not change if inventory is later edited, keeping historical documents accurate.
CREATE TABLE purchase_request_item (
    ...
    -- Snapshot of item name at line-creation time.
...
-- ==============================================
-- STEP 12: CREATE PURCHASE ORDER TABLES
-- ==============================================
...
-- Extends PR items with pricing columns and received_quantity.
-- Printed order is determined by poi_id ASC (insertion order).
-- item_description and uom are snapshot copies — historically stable.
CREATE TABLE purchase_order_item (
    ...
    -- How many units actually arrived on delivery.
    -- Starts at 0. Backend sets this to poi_quantity (or less for partial delivery)
    -- before transitioning PO status to 'received'.
    -- THIS value — not poi_quantity — is what gets written into stock_movement.
...
-- ==============================================
-- STEP 13: CREATE STOCK MOVEMENT TABLE
-- ==============================================
-- Full ledger of every stock change for every inventory item.
-- This is the source of truth for stock history and predictive analytics.
...
-- Used by backend to generate pr_no / po_no.
...
-- ==============================================
-- STEP 15: ROW LEVEL SECURITY (RLS)
-- ==============================================
...
-- Enum lookup (all authenticated users can read; restrict writes in production)
...
-- ==============================================
-- STEP 17: VERIFY SETUP (optional queries)
-- ==============================================
-- Check all public tables
-- SELECT table_name FROM information_schema.tables
-- WHERE table_schema = 'public' ORDER BY table_name;

-- Confirm system_enum seeded correctly
-- SELECT enum_group, enum_code, enum_label, sort_order, is_active
-- FROM system_enum ORDER BY enum_group, sort_order;

-- Confirm inventory has quantity + balance_qty, no quantity_id
-- SELECT column_name, data_type, column_default
-- FROM information_schema.columns
-- WHERE table_name = 'inventory' AND table_schema = 'public'
-- ORDER BY ordinal_position;

-- Confirm stock_movement structure
-- SELECT column_name, data_type, is_nullable
-- FROM information_schema.columns
-- WHERE table_name = 'stock_movement' AND table_schema = 'public'
-- ORDER BY ordinal_position;

-- Confirm purchase_order_item has poi_quantity + received_quantity, no item_no
-- SELECT column_name, data_type, column_default
-- FROM information_schema.columns
