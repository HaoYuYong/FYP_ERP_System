...
export interface SIPDFCustomer {
  company_name?: string;
...
export interface SIPDFItem {
  item_name: string;
  item_description?: string;
  uom?: string;
  si_quantity: number | string;
...
export interface SalesInvoicePDFProps {
  ...
  company: SIPDFCompany | null;
  customer: SIPDFCustomer | null;
  items: SIPDFItem[];
  printedBy: string;
  printedAt: Date;
}
...
const s = StyleSheet.create({
  ...
  // ═══════════════════════════════════════════════════
  // SECTION 2 — Document title + Customer / SI details
  // ═══════════════════════════════════════════════════
  docTitle: {
    fontSize: 18,
    fontFamily: 'Helvetica-Bold',
  ...
  colUom:       { width: 40 },
  colQty:       { width: 36 },
  colUnitPrice: { width: 56 },
  colLineTotal: { width: 62 },
...
const SalesInvoicePDF: React.FC<SalesInvoicePDFProps> = ({
  ...
  return (
    <Document title={`Sales Invoice ${siNo}`} author={printedBy}>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.pageFooter} fixed>
          <Text style={s.pageFooterText}>{siNo}</Text>
          <Text
        ...
        <View style={s.twoColRow}>
          ...
          {/* ── Right 40%: SI Reference box ── */}
          <View style={s.colRight}>
            <View style={s.refBox}>
              <View style={s.refRow}>
                <Text style={s.refLabel}>SI No.</Text>
                <Text style={s.refValue}>{fmt(siNo)}</Text>
              </View>
              <View style={s.refRow}>
              ...
              <View style={s.refRow}>
                <Text style={s.refLabel}>Due Date</Text>
                <Text style={s.refValue}>{fmtDate(dueDate)}</Text>
              </View>
              <View style={s.refRow}>
