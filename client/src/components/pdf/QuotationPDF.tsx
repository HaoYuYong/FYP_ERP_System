...
export interface QuotPDFCompany {
  company_name?: string;
  register_no?: string;
  address?: string;
  city?: string;
...
export interface QuotPDFCustomer {
  company_name?: string;
...
export interface QuotPDFItem {
  item_name: string;
  item_description?: string;
  uom?: string;
  qi_quantity: number | string;
...
export interface QuotationPDFProps {
  ...
  company: QuotPDFCompany | null;
  customer: QuotPDFCustomer | null;
  items: QuotPDFItem[];
  printedBy: string;
  printedAt: Date;
}
...
const s = StyleSheet.create({
  ...
  // ═══════════════════════════════════════════════════
  // SECTION 2 — Document title + Customer / QUOT details
  // ═══════════════════════════════════════════════════
  docTitle: {
    fontSize: 18,
    fontFamily: 'Helvetica-Bold',
...
const QuotationPDF: React.FC<QuotationPDFProps> = ({
  ...
  return (
    <Document title={`Quotation ${quotNo}`} author={printedBy}>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.pageFooter} fixed>
          <Text style={s.pageFooterText}>{quotNo}</Text>
          <Text
        ...
        <View style={s.twoColRow}>
          ...
          {/* ── Right 40%: QUOT Reference box ── */}
          <View style={s.colRight}>
            <View style={s.refBox}>
              <View style={s.refRow}>
                <Text style={s.refLabel}>QUOT No.</Text>
                <Text style={s.refValue}>{fmt(quotNo)}</Text>
              </View>
              <View style={s.refRow}>
